import {Component} from '@angular/core';
import {FormService} from '@apereo/mgmt-lib/src/lib/form';
import {OidcRegisteredService} from '@apereo/mgmt-lib/src/lib/model';
import {TabOidcForm} from './tab-oidc.form';

/**
 * Component to display/update OIDC options for a service.
 *
 * @author Travis Schmidt
 */
@Component({
  selector: 'lib-tab-oidc',
  templateUrl: './tab-oidc.component.html'
})
export class TabOIDCComponent {

  readonly key = 'oidc';
  get tab() { return this.service.form.get(this.key) as TabOidcForm; }
  set tab(f: TabOidcForm) { this.service.form.addControl(this.key, f); }

  /**
   * Starts the component by looking in the data record for an existing form for this tab. If not found
   * a new form is created and inserted in the map.
   */
  constructor(private service: FormService) {
    if (service.form.contains(this.key)) {
      this.tab = new TabOidcForm(service.registeredService as OidcRegisteredService);
    }
  }

}
